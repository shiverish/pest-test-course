    ...
    <AuthenticatedLayout>
        <template #header>
            <div class="flex items-center justify-between">
                <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
                    <Link :href="route('course.show', lesson.course_id)" class="text-indigo-500 hover:text-indigo-400 mr-2">
                        &larr; Courses
                    </Link>
                    / {{ lesson.title }}
                </h2>
                <div v-if="isCompleted" class="px-4 py-1.5 bg-green-500/10 border border-green-500/20 text-green-500 rounded-full font-bold text-sm tracking-wide flex items-center shadow-[0_0_15px_rgba(34,197,94,0.3)]">
        ...
        <div class="py-8 h-[calc(100vh-140px)] flex flex-col">
            <div class="max-w-8xl mx-auto sm:px-6 lg:px-8 w-full flex-grow flex flex-col lg:flex-row gap-6 h-full">
                ...
                <div class="w-full lg:w-1/3 flex flex-col bg-white dark:bg-gray-800 shadow-xl rounded-2xl overflow-hidden border border-gray-100 dark:border-gray-700">
                    <div class="p-6 bg-gray-50 dark:bg-gray-900 border-b border-gray-100 dark:border-gray-800 flex items-center">
                        ...
                        <h3 class="text-xl font-bold dark:text-white">Instructions</h3>
                    </div>
                    <div class="p-6 flex-grow overflow-y-auto prose dark:prose-invert relative">
                        <p class="text-gray-700 dark:text-gray-300 leading-relaxed text-lg mb-8">{{ lesson.content }}</p>
                        
                        <div v-if="lesson.hint" class="mt-8 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-700/50 rounded-xl p-5 shadow-sm">
                            <h4 class="text-amber-800 dark:text-amber-400 font-bold flex items-center mb-2">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z" />
                                </svg>
                                Hint
                            </h4>
                            <p class="text-amber-700 dark:text-amber-300/80 m-0 text-md">{{ lesson.hint }}</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="w-full lg:w-2/3 flex flex-col gap-6 h-full">
                    <!-- Editor -->
                    <div class="flex-grow flex flex-col bg-[#1e1e1e] rounded-2xl shadow-2xl overflow-hidden border border-gray-800 relative">
                        <div class="flex items-center px-4 py-3 bg-[#252526] border-b border-gray-900">
                            <div class="flex gap-2 mr-4">
                                <div class="w-3 h-3 rounded-full bg-red-500"></div>
                                <div class="w-3 h-3 rounded-full bg-yellow-500"></div>
                                <div class="w-3 h-3 rounded-full bg-green-500"></div>
                            </div>
                            <span class="text-gray-400 text-sm font-mono flex items-center text-sm">
                    ...
                    <!-- Output Terminal -->
                    <div class="h-48 bg-[#0d1117] rounded-2xl shadow-inner overflow-hidden border border-gray-800 flex flex-col relative group">
                        <div class="px-4 py-2 bg-[#161b22] border-b border-[#30363d] flex items-center">
